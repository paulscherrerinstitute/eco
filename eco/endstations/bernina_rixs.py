...
class Analyzer(Assembly):
    ...
    def energy_from_motor_pos(self, om, t_hor, det_t_hor, det_t_ver, *args):
        tth = self.tth_from_motor_pos(t_hor=t_hor, det_t_hor=det_t_hor, det_t_ver=det_t_ver)
        energy = xu.lam2en(self.material.planeDistance(*self.hkl)*2*np.sin(np.deg2rad(tth/2)))
        return energy
...
class RIXS(Assembly):
    def __init__(
        ...
        self.config={
                'crystals':{
                    'Si533':{
                        'xu': xu.HXRD(xu.materials.Si.Q(0,-1,1),xu.materials.Si.Q(5,3,3)),
                        'material': xu.materials.Si,
                    },
                    'Si844':{
                        'xu': xu.HXRD(xu.materials.Si.Q(0,-1,1),xu.materials.Si.Q(8,4,4)),
                        'material': xu.materials.Si,
                    }
                },
                'rowland': {
        ...
        # append an analyzer
        self.append_analyzer(pos=2, analyzer='Si533', hkl=(8,4,4), name='ana_2', det=self.__dict__['det'])

    def append_analyzer(self, pos=None, analyzer='Si533', hkl=(8,4,4), name=None, det=None):
        if not name:
            if not pos:
