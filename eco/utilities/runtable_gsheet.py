...
class RuntableGsheet:
    ...
    def get_available_keys(self):
        ks = self._spreadsheet.worksheet(self._wstitle_available_keys).get_values(
            ":".join(self._range_available_keys)
        )
    ...
    def set_available_keys(self, keys):
        ...
        shape = (
            rng["endColumnIndex"] - rng["startColumnIndex"],
            rng["endRowIndex"] - rng["startRowIndex"],
        )
        cells = []
        nrowstot = 0
        ncolstot = 0
    ...
    def fill_run_table_data(self, table):
        cell_list = self._spreadsheet.worksheet(self._wstitle_run_table).range(
            ":".join(self._range_run_table_keys)
        )
        set_cells = []
        nrowstot = 0
        ncolstot = 0
            ...
            if tstr in test_keys:
                ...
                for n, set_val in enumerate(set_vals):
                    try:
                        ...
                        pass
                    if not isinstance(set_val, Number) and not isinstance(set_val, str):
                        set_val = str(set_val)
