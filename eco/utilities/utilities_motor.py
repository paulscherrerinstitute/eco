...
def find_limit(adjustable_motor,direction,search_interval,abs_move_limit=9e5, speed=None):
    ...
    print( (not np.sum(np.sign(np.asarray(lims_old)-curr_pos))==0),((np.asarray(lims_old)-curr_pos)==0).any())
    if (not np.sum(np.sign(np.asarray(lims_old)-curr_pos))==0) or ((np.asarray(lims_old)-curr_pos)==0).any():
...
def find_limits(adjustable_motor,search_interval,abs_move_limit=9e5, speed=None):
    ...
    for direction in (-1,1):
        limits.append(find_limit(adjustable_motor,direction,search_interval,abs_move_limit=abs_move_limit,speed=speed))
